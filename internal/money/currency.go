...
// exponents maps an ISO 4217 alphabetic code to the number of fraction digits
// in one major unit. The table is a closed set on purpose: a code that is not
// in it gets rejected instead of silently defaulting to two digits, which is
// what turns a JPY row into a hundredfold overstatement.
//
// Fund, metal and test codes (XAU, XDR, XXX and friends) are left out: they are
// not payment currencies and have no business arriving in a transaction feed.
...
// Exponent is how many fraction digits one major unit of currency carries: 2
// for SAR, 0 for JPY, 3 for KWD. The code must already be upper case.
...
// Known reports whether the code names a currency this service can price.
